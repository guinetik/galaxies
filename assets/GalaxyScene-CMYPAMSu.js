import{W as Ae}from"./three.module-1IXlXC5N.js";import{m as De}from"./morphologyMapper-DirS7Qm3.js";import{c as ze,a as Re,r as Te,g as Pe,d as ke}from"./qualityDetect-BCnclKiE.js";import{c as Q,B as le,a as Y,A as oe,b as ce,m as _,h as Ie,x as te,$ as ge,y as ye,M as L,d as xe,ab as Ee,D as _e,u as Le,aS as Oe,L as J,w as he,f as de,N as Ne,K as ee,C as He,V as X,Q as $,S as ue,P as Fe,dh as Be,aQ as Ue}from"./three.core-Bozt1HMY.js";const b=Math.PI*2,O={rotation:{baseSpeed:.033,falloff:.35,referenceRadius:20},visual:{diskThicknessRatio:.06,dustFraction:.65,brightFraction:.03,dustHueRange:[240,280],brightHueRange:[10,45],hiiRegionChance:.15}},me=[{hue:10,spread:8,wInner:.35,wOuter:.05},{hue:25,spread:8,wInner:.3,wOuter:.1},{hue:42,spread:6,wInner:.25,wOuter:.15},{hue:55,spread:5,wInner:.08,wOuter:.2},{hue:210,spread:15,wInner:.02,wOuter:.35},{hue:225,spread:10,wInner:0,wOuter:.15}];function R(i){const e=O.visual.dustFraction,o=O.visual.brightFraction;return i<e?"dust":i>1-o?"bright":"star"}function T(i){switch(i){case"dust":return{size:.8+Math.random()*1.5,brightness:.08+Math.random()*.16,alpha:.12+Math.random()*.2};case"bright":return{size:4+Math.random()*6,brightness:.64+Math.random()*.16,alpha:.56+Math.random()*.24};default:return{size:1.5+Math.random()*3,brightness:.32+Math.random()*.4,alpha:.4+Math.random()*.4}}}function P(i,e,o){const t=O.visual;if(i==="dust")return t.dustHueRange[0]+Math.random()*(t.dustHueRange[1]-t.dustHueRange[0]);if(i==="bright")return t.brightHueRange[0]+Math.random()*(t.brightHueRange[1]-t.brightHueRange[0]);const n=Math.pow(e,.6);let s=0;for(const c of me)s+=c.wInner*(1-n)+c.wOuter*n;let a=Math.random()*s;for(const c of me)if(a-=c.wInner*(1-n)+c.wOuter*n,a<=0)return c.hue+(Math.random()-.5)*c.spread;return 42}function k(i){const{baseSpeed:e,falloff:o,referenceRadius:t}=O.rotation;return e/Math.pow(Math.max(i,t)/t,o)}function Ge(i){return i.galaxyRadius*.06}function Ve(i,e){const o=Ge(e);return i.filter(t=>t.radius>=o)}function ne(i){const e=Math.random()*b,o=Math.sqrt(Math.random())*i,t=(Math.random()-.5)*i*.08,n=R(Math.random()),s=T(n),a=o/i;return{radius:o,angle:e,y:t,rotationSpeed:k(o),hue:P(n,a),brightness:s.brightness,size:s.size,alpha:s.alpha,layer:n,twinklePhase:Math.random()*b}}function fe(i,e){const o=[],t=i.morphology,n=i.galaxyRadius,s=t.numArms,a=Math.floor(e/s),c=t.armWidth*n,u=t.spiralTightness,f=t.spiralStart*n,r=t.irregularity,d=t.barLength>0,g=t.barLength*n,m=2.5,p=Math.min(Math.max(f,d?g*.5:0),n*.98),y=p*p,h=n*n;for(let l=0;l<s;l++){const v=l/s*b;for(let M=0;M<a;M++){const S=Math.sqrt(Math.random()*(h-y)+y),w=Math.max(f,.001),C=Math.log(Math.max(S/w,1))/Math.max(u,.001)*m+v,W=S/n*.5+.5,q=(Math.random()-.5+Math.random()-.5)*c*W,j=C+Math.PI/2,x=r*(Math.random()-.5)*30,A=Math.cos(C)*(S+x)+Math.cos(j)*q,I=Math.sin(C)*(S+x)+Math.sin(j)*q,N=S/n,H=n*O.visual.diskThicknessRatio*(1-N*.7),F=(Math.random()-.5)*H,z=Math.sqrt(A*A+I*I),B=Math.atan2(I,A),U=z/n,E=k(z),G=R(Math.random()),V=T(G),Z=P(G,U);o.push({radius:z,angle:B,y:F,rotationSpeed:E,hue:Z,brightness:V.brightness,size:V.size,alpha:V.alpha,layer:G,twinklePhase:Math.random()*b})}}return o}function Xe(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.barLength*t,s=i.morphology.barWidth*t;for(let a=0;a<e;a++){const c=(Math.random()-.5)*2*n,u=(Math.random()-.5)*s,f=c,r=u,d=Math.sqrt(f*f+r*r);if(d>t)continue;const g=Math.atan2(r,f),m=R(Math.random()),p=T(m),y=P(m,.1);o.push({radius:d,angle:g,y:(Math.random()-.5)*t*.04,rotationSpeed:k(d),hue:y,brightness:p.brightness,size:p.size,alpha:p.alpha,layer:m,twinklePhase:Math.random()*b})}return o}function pe(i,e){const o=[],t=i.morphology.bulgeRadius*i.galaxyRadius;for(let n=0;n<e;n++){const s=Math.pow(Math.random(),.6)*t,a=Math.random()*b,c=(Math.random()-.5)*t*.5,u=s/t,f=1+(1-u)*.5,r=R(Math.random()),d=T(r);o.push({radius:s,angle:a,y:c,rotationSpeed:k(s)*.5,hue:P(r,.1),brightness:Math.min(d.brightness*f,.95),size:d.size*(1+(1-u)*.3),alpha:Math.min(d.alpha*f,.95),layer:r,twinklePhase:Math.random()*b})}return o}function Ye(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.axisRatio;for(let s=0;s<e;s++){const a=Math.random(),c=Math.random(),u=Math.pow(a,.4)*t,f=c*b,r=u*Math.cos(f),d=u*Math.sin(f)*n,g=Math.sqrt(r*r+d*d),m=Math.atan2(d,r),p=g/t,y=R(Math.random()),h=T(y),l=P(y,p);o.push({radius:g,angle:m,y:(Math.random()-.5)*t*.1*(1-p*.5),rotationSpeed:k(g)*.3,hue:l,brightness:h.brightness,size:h.size,alpha:h.alpha,layer:y,twinklePhase:Math.random()*b})}return o}function We(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.bulgeRadius*t;for(let s=0;s<e;s++){const a=Math.pow(Math.random(),.55)*t,c=Math.random()*b,u=a/t,f=t*.06*Math.pow(Math.max(1-u,0),2),r=(Math.random()-.5)*f,d=Math.max(0,Math.min(1,1-a/Math.max(n,1))),g=1+d*.4,m=R(Math.random()),p=T(m);o.push({radius:a,angle:c,y:r,rotationSpeed:k(a)*(d>0?.5:1),hue:P(m,u*.2),brightness:Math.min(p.brightness*g,.95),size:p.size*(1+d*.3),alpha:Math.min(p.alpha*g,.95),layer:m,twinklePhase:Math.random()*b})}return o}function qe(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.irregularity,s=i.morphology.clumpCount,a=[];for(let c=0;c<s;c++){const u=c/s*b+Math.random()*.5,f=(.2+Math.random()*.6)*t;a.push({x:Math.cos(u)*f,z:Math.sin(u)*f,sigma:30+Math.random()*80,weight:.5+Math.random(),isHII:Math.random()<O.visual.hiiRegionChance})}for(let c=0;c<e;c++){let u,f;if(Math.random()<1-n){const h=Math.floor(Math.random()*s),l=a[h],v=()=>(Math.random()-.5+Math.random()-.5)*2;u=l.x+v()*l.sigma,f=l.z+v()*l.sigma,l.isHII&&Math.random()<.4}else{const h=Math.random()*b,l=Math.sqrt(Math.random())*t;u=Math.cos(h)*l+(Math.random()-.5)*60,f=Math.sin(h)*l+(Math.random()-.5)*60}const r=Math.sqrt(u*u+f*f);if(r>t*1.1)continue;const d=Math.atan2(f,u),g=r/t,m=R(Math.random()),p=T(m),y=P(m,g);o.push({radius:r,angle:d,y:(Math.random()-.5)*t*.12,rotationSpeed:k(r)*(.5+Math.random()*.5),hue:y,brightness:p.brightness,size:p.size,alpha:p.alpha,layer:m,twinklePhase:Math.random()*b})}return o}function je(i){const e=i.morphology,o=i.starCount,t=i.galaxyRadius;let n=[];const s=e.barLength>0,a=e.numArms>0,c=e.clumpCount>0&&e.irregularity>0,u=e.ellipticity>0&&!a&&!s&&!c,f=!a&&!s&&!c&&e.ellipticity===0&&e.bulgeFraction>0;if(u)n.push(...Ye(i,o));else if(f)n.push(...We(i,o));else if(c){const r=Math.floor(o*e.fieldStarFraction),d=o-r;n.push(...qe(i,d));for(let g=0;g<r;g++)n.push(ne(t))}else if(s&&a){const r=Math.floor(o*.25),d=o-r;n.push(...Xe(i,r));const g=Math.floor(d*.9);n.push(...fe(i,g));const m=e.bulgeRadius*t;if(m>0){const y=Math.min(.2,.08+.18*(m/t)),h=Math.floor(o*y);n.push(...pe(i,h))}const p=Math.floor(d*.1);for(let y=0;y<p;y++)n.push(ne(t))}else if(a){const r=e.fieldStarFraction,d=Math.floor(o*(1-r));n.push(...fe(i,d));const g=e.bulgeRadius*t;if(g>0){const p=Math.min(.25,.1+.2*(g/t)),y=Math.floor(o*p);n.push(...pe(i,y))}const m=Math.floor(o*r);for(let p=0;p<m;p++)n.push(ne(t))}return Ve(n,i)}const Ze=`attribute float aSize;
...
`;class at{constructor(e,o,t){const n=e*12,s=t==="mobile",a=s?[48,32]:[192,128],c=new xe(n,a[0],a[1]);this.material=new Y({vertexShader:nt,fragmentShader:ot,defines:{SPIRAL_NOISE_ITER:s?3:5,MAX_GALAXIES:s?2:4,MAX_CLOUDS:s?3:6,MAX_KNOTS:s?2:5,STAR_LAYERS:s?2:4,FBM_DETAIL_OCTAVES:s?2:4},uniforms:{uTime:{value:0},uSeed:{value:o},uNebulaIntensity:{value:2.4}},side:Ee,depthWrite:!1,depthTest:!1}),this.mesh=new L(c,this.material),this.mesh.frustumCulled=!1,this.mesh.renderOrder=-10}update(e,o){this.material.uniforms.uTime.value=e,this.mesh.position.copy(o.position)}dispose(){this.material.dispose(),this.mesh.geometry.dispose()}}const st=`varying vec2 vUV;
...
`;class rt{constructor(e,o,t,n){const a=new Float32Array(65536),c=o*1.3;for(let m=0;m<e.length;m++){const p=e[m],y=Math.cos(p.angle)*p.radius,h=Math.sin(p.angle)*p.radius,l=Math.floor((y/c*.5+.5)*255),v=Math.floor((h/c*.5+.5)*255);l>=0&&l<256&&v>=0&&v<256&&(a[v*256+l]+=1)}const u=new Float32Array(256*256);for(let m=0;m<3;m++){const p=m%2===0?a:u,y=m%2===0?u:a;for(let h=0;h<256;h++)for(let l=0;l<256;l++){let v=0,M=0;for(let S=-2;S<=2;S++)for(let w=-2;w<=2;w++){const D=l+w,C=h+S;D>=0&&D<256&&C>=0&&C<256&&(v+=p[C*256+D],M++)}y[h*256+l]=v/M}}a.set(u);let f=0;for(let m=0;m<a.length;m++)a[m]>f&&(f=a[m]);const r=new Uint8Array(256*256);if(f>0)for(let m=0;m<a.length;m++)r[m]=Math.min(255,Math.floor(a[m]/f*255));this.densityTexture=new _e(r,256,256,Le,Oe),this.densityTexture.minFilter=J,this.densityTexture.magFilter=J,this.densityTexture.wrapS=he,this.densityTexture.wrapT=he,this.densityTexture.needsUpdate=!0;const d=new te(2,2),g=n==="mobile";this.material=new Y({vertexShader:st,fragmentShader:it,defines:{FBM_MAX_OCTAVES:g?2:4,SPIRAL_ITERS:g?3:5},uniforms:{uInvViewProj:{value:new de},uTime:{value:0},uGalaxyRadius:{value:o},uSeed:{value:t},uNebulaIntensity:{value:.4},uGalaxyRotation:{value:0},uAxisRatio:{value:1},uDensityMap:{value:this.densityTexture}},transparent:!0,depthWrite:!1,depthTest:!1,blending:oe}),this.mesh=new L(d,this.material),this.mesh.frustumCulled=!1,this.mesh.renderOrder=-1}update(e,o,t,n,s){const a=this.material.uniforms;a.uTime.value=e,a.uGalaxyRotation.value=t,a.uGalaxyRadius.value=n,a.uAxisRatio.value=s;const c=new de;c.multiplyMatrices(o.projectionMatrix,o.matrixWorldInverse),a.uInvViewProj.value.copy(c).invert()}dispose(){this.densityTexture.dispose(),this.material.dispose(),this.mesh.geometry.dispose()}}const lt=`varying vec2 vUV;
void main() {
  vUV = uv;
  gl_Position = projectionMatrix * modelViewMatrix * vec4(position, 1.0);
}
...
`,ve=new X(0,1,0),K=new $;class gt{constructor(e,o){this.animationId=0,this.clock=new He,this.galaxyRotation=0,this._bhScreenVec=new X,this.orbitQuat=new $,this.zoom=4,this.targetZoom=4,this.isDragging=!1,this.isPinching=!1,this.lastX=0,this.lastY=0,this.velocityX=0,this.velocityY=0,this.lastPinchDist=0,this.renderer=new Ae({canvas:e,antialias:!0,alpha:!0});const t=ke();this.renderer.setPixelRatio(Math.min(window.devicePixelRatio,Re(t))),this.renderer.setSize(e.clientWidth,e.clientHeight,!1),this.scene=new ue,this.renderer.setClearColor(0,1),this.params=De(o);const n=je(this.params),s=this.params.galaxyRadius;this.baseDistance=s*1.7;const a=e.clientWidth/e.clientHeight;this.camera=new Fe(60,a,.1,this.baseDistance*20),this.backdrop=new at(this.baseDistance,o.pgc,t),this.scene.add(this.backdrop.mesh),this.particles=new et(n,this.baseDistance),this.scene.add(this.particles.points),this.scene.add(this.particles.foregroundPoints),this.haze=new tt(s),this.scene.add(this.haze.mesh),this.nebula=new rt(n,s,o.pgc,t),this.scene.add(this.nebula.mesh),this.blackHole=new ht(null,s*.08),this.scene.add(this.blackHole.depthMesh),this.scene.add(this.blackHole.mesh),this.backdrop.mesh.layers.set(1),this.particles.points.layers.set(1),this.particles.foregroundPoints.layers.set(2),this.haze.mesh.layers.set(1),this.nebula.mesh.layers.set(1);const c=e.clientWidth,u=e.clientHeight;this.rtScaleFactor=Te(t),this.galaxyRT=new Be(c*this.renderer.getPixelRatio()*this.rtScaleFactor,u*this.renderer.getPixelRatio()*this.rtScaleFactor,{minFilter:J,magFilter:J}),this.lensingMaterial=new Y({vertexShader:dt,fragmentShader:ut,uniforms:{uSceneTexture:{value:this.galaxyRT.texture},uBHScreenPos:{value:new ee(.5,.5)},uLensStrength:{value:0},uLensZoom:{value:0},uAspectRatio:{value:c/u}},depthTest:!1,depthWrite:!1});const f=new L(new te(2,2),this.lensingMaterial);this.lensingScene=new ue,this.lensingScene.add(f),this.lensingCamera=new Ue(-1,1,1,-1,0,1);const d=typeof window<"u"&&window.innerWidth<768?2:4;this.zoom=d,this.targetZoom=d;const{initRotY:g,initTiltX:m}=Pe(o.pgc),p=new $().setFromAxisAngle(new X(1,0,0),m),y=new $().setFromAxisAngle(ve,g);this.orbitQuat.multiplyQuaternions(y,p),this.onPointerDown=h=>{this.isPinching||(this.isDragging=!0,this.lastX=h.clientX,this.lastY=h.clientY,this.velocityX=0,this.velocityY=0)},this.onPointerMove=h=>{if(this.isPinching||!this.isDragging)return;const l=h.clientX-this.lastX,v=h.clientY-this.lastY;this.velocityX=l*.005,this.velocityY=v*.005,this.applyOrbitDelta(this.velocityX,this.velocityY),this.lastX=h.clientX,this.lastY=h.clientY},this.onPointerUp=()=>{this.isDragging=!1},this.onPointerCancel=()=>{this.isDragging=!1,this.isPinching=!1},this.onWheel=h=>{h.preventDefault();const l=this.targetZoom*.12;this.targetZoom+=h.deltaY>0?-l:l,this.targetZoom=Math.max(.1,Math.min(20,this.targetZoom))},this.onTouchStart=h=>{if(h.touches.length===2){h.preventDefault(),this.isPinching=!0,this.isDragging=!1;const l=h.touches[0].clientX-h.touches[1].clientX,v=h.touches[0].clientY-h.touches[1].clientY;this.lastPinchDist=Math.sqrt(l*l+v*v)}},this.onTouchMove=h=>{if(h.touches.length===2){h.preventDefault();const l=h.touches[0].clientX-h.touches[1].clientX,v=h.touches[0].clientY-h.touches[1].clientY,M=Math.sqrt(l*l+v*v),S=(M-this.lastPinchDist)*.01;this.lastPinchDist=M,this.targetZoom=Math.max(.1,Math.min(20,this.targetZoom+S))}},this.onTouchEnd=()=>{this.lastPinchDist>0&&(this.lastPinchDist=0),this.isPinching=!1},e.addEventListener("pointerdown",this.onPointerDown),e.addEventListener("pointermove",this.onPointerMove),e.addEventListener("pointerup",this.onPointerUp),e.addEventListener("pointercancel",this.onPointerCancel),e.addEventListener("pointerleave",this.onPointerUp),e.addEventListener("wheel",this.onWheel,{passive:!1}),e.addEventListener("touchstart",this.onTouchStart,{passive:!1}),e.addEventListener("touchmove",this.onTouchMove,{passive:!1}),e.addEventListener("touchend",this.onTouchEnd),this.resizeObserver=new ResizeObserver(()=>{const h=e.clientWidth,l=e.clientHeight;if(h===0||l===0)return;this.renderer.setSize(h,l,!1),this.camera.aspect=h/l,this.camera.updateProjectionMatrix();const v=this.renderer.getPixelRatio();this.galaxyRT.setSize(h*v*this.rtScaleFactor,l*v*this.rtScaleFactor),this.lensingMaterial.uniforms.uAspectRatio.value=h/l}),this.resizeObserver.observe(e)}applyOrbitDelta(e,o){K.setFromAxisAngle(ve,-e),this.orbitQuat.premultiply(K);const t=new X(1,0,0).applyQuaternion(this.orbitQuat);K.setFromAxisAngle(t,-o),this.orbitQuat.premultiply(K),this.orbitQuat.normalize()}renderGalaxyPostPass(e,o,t,n){this.camera.layers.set(1),this.renderer.setRenderTarget(this.galaxyRT),this.renderer.clear(),this.renderer.render(this.scene,this.camera),this.lensingMaterial.uniforms.uBHScreenPos.value.set(e,o),this.lensingMaterial.uniforms.uLensStrength.value=t,this.lensingMaterial.uniforms.uLensZoom.value=n,this.renderer.setRenderTarget(null),this.renderer.clear(),this.renderer.render(this.lensingScene,this.lensingCamera)}start(){this.clock.start();const e=()=>{this.animationId=requestAnimationFrame(e);const o=this.clock.getDelta(),t=this.clock.getElapsedTime();this.isDragging||(Math.abs(this.velocityX)>1e-4||Math.abs(this.velocityY)>1e-4)&&(this.applyOrbitDelta(this.velocityX,this.velocityY),this.velocityX*=.92,this.velocityY*=.92),this.zoom+=(this.targetZoom-this.zoom)*.08;const n=this.baseDistance/this.zoom,s=new X(0,0,n).applyQuaternion(this.orbitQuat);this.camera.position.copy(s),this.camera.lookAt(0,0,0),this.camera.updateMatrixWorld(!0);const a=Math.min(this.zoom/20,1),c=.02+.18*a*a;this.galaxyRotation+=o*c;const u=this.camera.position,f=Math.sqrt(u.x*u.x+u.z*u.z),r=Math.atan2(u.y,f),d=Math.atan2(u.x,u.z);this.backdrop.update(t,this.camera),this.blackHole.update(t,r,d,this.camera,this.renderer),this._bhScreenVec.set(0,0,0).project(this.camera);const g=this._bhScreenVec.x*.5+.5,m=this._bhScreenVec.y*.5+.5,p=this.renderer.getSize(new ee),y=this.renderer.getPixelRatio();this.particles.update(o,t,this.camera,this._bhScreenVec.x,this._bhScreenVec.y,this.blackHole.getApparentPx(),p.x*y,p.y*y);const h=this.params.morphology.ellipticity>0?this.params.morphology.axisRatio:1;this.nebula.update(t,this.camera,this.galaxyRotation,this.params.galaxyRadius,h);{const l=this.blackHole.getLOD(),v=l*l*.045;this.renderGalaxyPostPass(g,m,v,l)}this.camera.layers.set(2),this.renderer.autoClear=!1,this.renderer.render(this.scene,this.camera),this.renderer.autoClear=!0};e()}dispose(){cancelAnimationFrame(this.animationId);const e=this.renderer.domElement;e.removeEventListener("pointerdown",this.onPointerDown),e.removeEventListener("pointermove",this.onPointerMove),e.removeEventListener("pointerup",this.onPointerUp),e.removeEventListener("pointercancel",this.onPointerCancel),e.removeEventListener("pointerleave",this.onPointerUp),e.removeEventListener("wheel",this.onWheel),e.removeEventListener("touchstart",this.onTouchStart),e.removeEventListener("touchmove",this.onTouchMove),e.removeEventListener("touchend",this.onTouchEnd),this.resizeObserver.disconnect(),this.backdrop.dispose(),this.particles.dispose(),this.haze.dispose(),this.nebula.dispose(),this.blackHole.dispose(),this.galaxyRT.dispose(),this.lensingMaterial.dispose(),this.renderer.dispose()}}export{gt as GalaxyScene};
