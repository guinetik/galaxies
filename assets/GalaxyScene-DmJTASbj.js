import{W as Ae}from"./three.module-BN_osZwL.js";import{m as De}from"./morphologyMapper-DirS7Qm3.js";import{c as ze,a as Re,r as Te,g as Pe,d as ke}from"./qualityDetect-BlUgYf3e.js";import{c as Z,B as le,a as W,A as ae,b as ce,z as N,j as Ie,q as ee,$ as ge,r as ye,M as H,d as xe,ab as Ee,D as _e,m as Le,aS as Oe,o as $,p as he,f as de,N as Ne,w as J,C as He,V as Y,Q as K,S as ue,P as Fe,dh as Be,aQ as Ue}from"./three.core-Dw7Kmcmn.js";const b=Math.PI*2,F={rotation:{baseSpeed:.033,falloff:.35,referenceRadius:20},visual:{diskThicknessRatio:.06,dustFraction:.65,brightFraction:.03,dustHueRange:[240,280],brightHueRange:[10,45],hiiRegionChance:.15}},me=[{hue:10,spread:8,wInner:.35,wOuter:.05},{hue:25,spread:8,wInner:.3,wOuter:.1},{hue:42,spread:6,wInner:.25,wOuter:.15},{hue:55,spread:5,wInner:.08,wOuter:.2},{hue:210,spread:15,wInner:.02,wOuter:.35},{hue:225,spread:10,wInner:0,wOuter:.15}];function T(i){const e=F.visual.dustFraction,o=F.visual.brightFraction;return i<e?"dust":i>1-o?"bright":"star"}function P(i){switch(i){case"dust":return{size:.8+Math.random()*1.5,brightness:.08+Math.random()*.16,alpha:.12+Math.random()*.2};case"bright":return{size:4+Math.random()*6,brightness:.64+Math.random()*.16,alpha:.56+Math.random()*.24};default:return{size:1.5+Math.random()*3,brightness:.32+Math.random()*.4,alpha:.4+Math.random()*.4}}}function k(i,e,o){const t=F.visual;if(i==="dust")return t.dustHueRange[0]+Math.random()*(t.dustHueRange[1]-t.dustHueRange[0]);if(i==="bright")return t.brightHueRange[0]+Math.random()*(t.brightHueRange[1]-t.brightHueRange[0]);const n=Math.pow(e,.6);let s=0;for(const c of me)s+=c.wInner*(1-n)+c.wOuter*n;let a=Math.random()*s;for(const c of me)if(a-=c.wInner*(1-n)+c.wOuter*n,a<=0)return c.hue+(Math.random()-.5)*c.spread;return 42}function I(i){const{baseSpeed:e,falloff:o,referenceRadius:t}=F.rotation;return e/Math.pow(Math.max(i,t)/t,o)}function Ge(i){return i.galaxyRadius*.06}function Ve(i,e){const o=Ge(e);return i.filter(t=>t.radius>=o)}function oe(i){const e=Math.random()*b,o=Math.sqrt(Math.random())*i,t=(Math.random()-.5)*i*.08,n=T(Math.random()),s=P(n),a=o/i;return{radius:o,angle:e,y:t,rotationSpeed:I(o),hue:k(n,a),brightness:s.brightness,size:s.size,alpha:s.alpha,layer:n,twinklePhase:Math.random()*b}}function fe(i,e){const o=[],t=i.morphology,n=i.galaxyRadius,s=t.numArms,a=Math.floor(e/s),c=t.armWidth*n,u=t.spiralTightness,f=t.spiralStart*n,r=t.irregularity,d=t.barLength>0,g=t.barLength*n,m=2.5,p=Math.min(Math.max(f,d?g*.5:0),n*.98),y=p*p,h=n*n;for(let l=0;l<s;l++){const v=l/s*b;for(let M=0;M<a;M++){const S=Math.sqrt(Math.random()*(h-y)+y),w=Math.max(f,.001),C=Math.log(Math.max(S/w,1))/Math.max(u,.001)*m,D=(Math.random()-.5)*.3,E=C+v+D,te=S/n*.5+.5,q=(Math.random()-.5+Math.random()-.5)*c*te,x=E+Math.PI/2,A=r*(Math.random()-.5)*30,_=Math.cos(E)*(S+A)+Math.cos(x)*q,z=Math.sin(E)*(S+A)+Math.sin(x)*q,B=S/n,U=n*F.visual.diskThicknessRatio*(1-B*.7),G=(Math.random()-.5)*U,R=Math.sqrt(_*_+z*z),V=Math.atan2(z,_),L=R/n,ne=I(R),X=T(Math.random()),O=P(X),j=k(X,L);o.push({radius:R,angle:V,y:G,rotationSpeed:ne,hue:j,brightness:O.brightness,size:O.size,alpha:O.alpha,layer:X,twinklePhase:Math.random()*b})}}return o}function Xe(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.barLength*t,s=i.morphology.barWidth*t;for(let a=0;a<e;a++){const c=(Math.random()-.5)*2*n,u=(Math.random()-.5)*s,f=c,r=u,d=Math.sqrt(f*f+r*r);if(d>t)continue;const g=Math.atan2(r,f),m=T(Math.random()),p=P(m),y=k(m,.1);o.push({radius:d,angle:g,y:(Math.random()-.5)*t*.04,rotationSpeed:I(d),hue:y,brightness:p.brightness,size:p.size,alpha:p.alpha,layer:m,twinklePhase:Math.random()*b})}return o}function pe(i,e){const o=[],t=i.morphology.bulgeRadius*i.galaxyRadius;for(let n=0;n<e;n++){const s=Math.pow(Math.random(),.6)*t,a=Math.random()*b,c=(Math.random()-.5)*t*.5,u=s/t,f=1+(1-u)*.5,r=T(Math.random()),d=P(r);o.push({radius:s,angle:a,y:c,rotationSpeed:I(s)*.5,hue:k(r,.1),brightness:Math.min(d.brightness*f,.95),size:d.size*(1+(1-u)*.3),alpha:Math.min(d.alpha*f,.95),layer:r,twinklePhase:Math.random()*b})}return o}function Ye(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.axisRatio;for(let s=0;s<e;s++){const a=Math.random(),c=Math.random(),u=Math.pow(a,.4)*t,f=c*b,r=u*Math.cos(f),d=u*Math.sin(f)*n,g=Math.sqrt(r*r+d*d),m=Math.atan2(d,r),p=g/t,y=T(Math.random()),h=P(y),l=k(y,p);o.push({radius:g,angle:m,y:(Math.random()-.5)*t*.1*(1-p*.5),rotationSpeed:I(g)*.3,hue:l,brightness:h.brightness,size:h.size,alpha:h.alpha,layer:y,twinklePhase:Math.random()*b})}return o}function We(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.bulgeRadius*t;for(let s=0;s<e;s++){const a=Math.pow(Math.random(),.55)*t,c=Math.random()*b,u=a/t,f=t*.06*Math.pow(Math.max(1-u,0),2),r=(Math.random()-.5)*f,d=Math.max(0,Math.min(1,1-a/Math.max(n,1))),g=1+d*.4,m=T(Math.random()),p=P(m);o.push({radius:a,angle:c,y:r,rotationSpeed:I(a)*(d>0?.5:1),hue:k(m,u*.2),brightness:Math.min(p.brightness*g,.95),size:p.size*(1+d*.3),alpha:Math.min(p.alpha*g,.95),layer:m,twinklePhase:Math.random()*b})}return o}function qe(i,e){const o=[],t=i.galaxyRadius,n=i.morphology.irregularity,s=i.morphology.clumpCount,a=[];for(let c=0;c<s;c++){const u=c/s*b+Math.random()*.5,f=(.2+Math.random()*.6)*t;a.push({x:Math.cos(u)*f,z:Math.sin(u)*f,sigma:30+Math.random()*80,weight:.5+Math.random(),isHII:Math.random()<F.visual.hiiRegionChance})}for(let c=0;c<e;c++){let u,f;if(Math.random()<1-n){const h=Math.floor(Math.random()*s),l=a[h],v=()=>(Math.random()-.5+Math.random()-.5)*2;u=l.x+v()*l.sigma,f=l.z+v()*l.sigma,l.isHII&&Math.random()<.4}else{const h=Math.random()*b,l=Math.sqrt(Math.random())*t;u=Math.cos(h)*l+(Math.random()-.5)*60,f=Math.sin(h)*l+(Math.random()-.5)*60}const r=Math.sqrt(u*u+f*f);if(r>t*1.1)continue;const d=Math.atan2(f,u),g=r/t,m=T(Math.random()),p=P(m),y=k(m,g);o.push({radius:r,angle:d,y:(Math.random()-.5)*t*.12,rotationSpeed:I(r)*(.5+Math.random()*.5),hue:y,brightness:p.brightness,size:p.size,alpha:p.alpha,layer:m,twinklePhase:Math.random()*b})}return o}function je(i){const e=i.morphology,o=i.starCount,t=i.galaxyRadius;let n=[];const s=e.barLength>0,a=e.numArms>0,c=e.clumpCount>0&&e.irregularity>0,u=e.ellipticity>0&&!a&&!s&&!c,f=!a&&!s&&!c&&e.ellipticity===0&&e.bulgeFraction>0;if(u)n.push(...Ye(i,o));else if(f)n.push(...We(i,o));else if(c){const r=Math.floor(o*e.fieldStarFraction),d=o-r;n.push(...qe(i,d));for(let g=0;g<r;g++)n.push(oe(t))}else if(s&&a){const r=Math.floor(o*.25),d=o-r;n.push(...Xe(i,r));const g=Math.floor(d*.9);n.push(...fe(i,g));const m=e.bulgeRadius*t;if(m>0){const y=Math.min(.2,.08+.18*(m/t)),h=Math.floor(o*y);n.push(...pe(i,h))}const p=Math.floor(d*.1);for(let y=0;y<p;y++)n.push(oe(t))}else if(a){const r=e.fieldStarFraction,d=Math.floor(o*(1-r));n.push(...fe(i,d));const g=e.bulgeRadius*t;if(g>0){const p=Math.min(.25,.1+.2*(g/t)),y=Math.floor(o*p);n.push(...pe(i,y))}const m=Math.floor(o*r);for(let p=0;p<m;p++)n.push(oe(t))}return Ve(n,i)}const Ze=`attribute float aSize;
...
`;class at{constructor(e,o,t){const n=e*12,s=t==="mobile",a=s?[48,32]:[192,128],c=new xe(n,a[0],a[1]);this.material=new W({vertexShader:nt,fragmentShader:ot,defines:{SPIRAL_NOISE_ITER:s?3:5,MAX_GALAXIES:s?2:4,MAX_CLOUDS:s?3:6,MAX_KNOTS:s?2:5,STAR_LAYERS:s?2:4,FBM_DETAIL_OCTAVES:s?2:4},uniforms:{uTime:{value:0},uSeed:{value:o},uNebulaIntensity:{value:2.4}},side:Ee,depthWrite:!1,depthTest:!1}),this.mesh=new H(c,this.material),this.mesh.frustumCulled=!1,this.mesh.renderOrder=-10}update(e,o){this.material.uniforms.uTime.value=e,this.mesh.position.copy(o.position)}dispose(){this.material.dispose(),this.mesh.geometry.dispose()}}const st=`varying vec2 vUV;
