import{g as j,L as Z,u as Y,e as K}from"./three.module-CEsxlxCu.js";const h=256,u=Math.PI*2,o=h/2,T=[{hue:12,spread:10,wInner:.35,wOuter:.12},{hue:26,spread:10,wInner:.31,wOuter:.18},{hue:40,spread:8,wInner:.18,wOuter:.16},{hue:58,spread:6,wInner:.07,wOuter:.14},{hue:214,spread:9,wInner:.03,wOuter:.14},{hue:282,spread:12,wInner:.06,wOuter:.22}];function Q(e){const n=Math.pow(e,.6);let i=0;for(const a of T)i+=a.wInner*(1-n)+a.wOuter*n;let t=Math.random()*i;for(const a of T)if(t-=a.wInner*(1-n)+a.wOuter*n,t<=0)return a.hue+(Math.random()-.5)*a.spread;return 42}function L(e,n,i){e=(e%360+360)%360;const t=(1-Math.abs(2*i-1))*n,a=t*(1-Math.abs(e/60%2-1)),r=i-t/2;let c=0,s=0,l=0;return e<60?(c=t,s=a):e<120?(c=a,s=t):e<180?(s=t,l=a):e<240?(s=a,l=t):e<300?(c=a,l=t):(c=t,l=a),[Math.round((c+r)*255),Math.round((s+r)*255),Math.round((l+r)*255)]}function _(e,n,i,t,a,r,c){const[s,l,d]=L(t,.42,Math.min(.9,a)),g=e.createRadialGradient(n,i,0,n,i,c);g.addColorStop(0,`rgba(${s},${l},${d},${r})`),g.addColorStop(.33,`rgba(${s},${l},${d},${r*.42})`),g.addColorStop(1,`rgba(${s},${l},${d},0)`),e.fillStyle=g,e.beginPath(),e.arc(n,i,c,0,u),e.fill()}function b(){return(Math.random()-.5+Math.random()-.5)*2}function q(e){const n=(e%360+360)%360;return n>=72&&n<=186?n<129?66:260:n}function y(e,n,i,t,a,r=1,c=0){e.save(),e.translate(o,o),e.rotate(c),e.scale(1,r);const[s,l,d]=L(n,.28,i),g=e.createRadialGradient(0,0,0,0,0,a);g.addColorStop(0,`rgba(${s},${l},${d},${t})`),g.addColorStop(.45,`rgba(${s},${l},${d},${t*.32})`),g.addColorStop(1,`rgba(${s},${l},${d},0)`),e.fillStyle=g,e.beginPath(),e.arc(0,0,a,0,u),e.fill(),e.restore()}function v(e,n){const i=e.getImageData(0,0,h,h),t=i.data;for(let a=0;a<t.length;a+=4){if(t[a+3]===0)continue;const r=(Math.random()-.5)*n;t[a]=Math.max(0,Math.min(255,t[a]+r)),t[a+1]=Math.max(0,Math.min(255,t[a+1]+r)),t[a+2]=Math.max(0,Math.min(255,t[a+2]+r))}e.putImageData(i,0,0)}function p(e,n){const i=e.getImageData(0,0,h,h),t=i.data,a=Math.cos(n.rotation),r=Math.sin(n.rotation),c=n.majorRadius*n.axisRatio;for(let s=0;s<h;s++)for(let l=0;l<h;l++){const d=l+.5-o,g=s+.5-o,x=d*a-g*r,w=d*r+g*a,R=x/n.majorRadius,S=w/c,m=Math.sqrt(R*R+S*S);if(m>1.2)continue;const C=Math.atan2(S,R),H=Math.sin(C*n.armCount+m*13),D=Math.sin(C*1.7+m*6),I=1+n.irregularity*(H*.35+D*.22),E=Math.exp(-Math.pow(Math.max(0,m*n.concentration),1.2)),P=Math.exp(-Math.pow(Math.max(0,m*1.35),1.55)),B=Math.exp(-Math.pow(Math.abs(S)*12,1.35))*(1-m),N=Math.max(0,(E*.72+P*.46)*I),z=1-n.dustStrength*B,M=N*z;if(M<.01)continue;const O=Math.min(1,Math.max(0,Math.pow(m,.85))),U=n.centerHue+(n.outerHue-n.centerHue)*O+(Math.random()-.5)*2.5,F=q(U),k=Math.min(.86,.14+M*.72),[X,$,W]=L(F,.28,k),V=Math.min(1,M*.72),f=(s*h+l)*4;t[f]=Math.max(t[f],X),t[f+1]=Math.max(t[f+1],$),t[f+2]=Math.max(t[f+2],W),t[f+3]=Math.min(255,Math.max(t[f+3],Math.floor(V*255)))}e.putImageData(i,0,0)}function J(e){const n=Math.random()*u;p(e,{majorRadius:o*.84,axisRatio:.68+Math.random()*.2,rotation:n,centerHue:24,outerHue:284,concentration:1.22,irregularity:.8,dustStrength:.28,armCount:2}),y(e,39,.82,.6,o*.11,.8,n);for(let i=0;i<260;i++){const t=Math.pow(Math.random(),.55)*o*.86,a=Math.random()*u;_(e,o+Math.cos(a)*t,o+Math.sin(a)*t*.8,Q(t/(o*.86)),.42+Math.random()*.28,.07+Math.random()*.13,.45+Math.random()*1.4)}v(e,6)}function ee(e){const n=Math.random()*u;p(e,{majorRadius:o*.83,axisRatio:.62+Math.random()*.22,rotation:n,centerHue:20,outerHue:276,concentration:1.18,irregularity:.7,dustStrength:.24,armCount:2}),e.save(),e.translate(o,o),e.rotate(n);for(let i=0;i<260;i++){const t=(Math.random()-.5)*o*.75,a=b()*o*.03;_(e,t,a,42+(Math.random()-.5)*8,.66,.15,.45+Math.random()*1.2)}e.restore(),v(e,6)}function ne(e){const n=Math.random()*u;p(e,{majorRadius:o*.78,axisRatio:.6+Math.random()*.25,rotation:n,centerHue:32,outerHue:352,concentration:1.55,irregularity:.24,dustStrength:.1,armCount:1}),y(e,37,.92,.55,o*.12,.78,n),v(e,6)}function ae(e){const n=Math.random()*u;p(e,{majorRadius:o*.8,axisRatio:.35+Math.random()*.16,rotation:n,centerHue:28,outerHue:350,concentration:1.35,irregularity:.18,dustStrength:.2,armCount:1}),y(e,36,.9,.5,o*.09,.48,n),v(e,6)}function te(e){const i=o*.7;p(e,{majorRadius:o*.74,axisRatio:.7+Math.random()*.25,rotation:Math.random()*u,centerHue:18,outerHue:286,concentration:.95,irregularity:1,dustStrength:.14,armCount:3});const t=4+Math.floor(Math.random()*4),a=Array.from({length:t},()=>({x:o+(Math.random()-.5)*i*1.4,y:o+(Math.random()-.5)*i*1.4,sigma:15+Math.random()*25,hue:210+(Math.random()-.5)*30}));for(let r=0;r<900;r++){let c,s,l;if(Math.random()<.8){const d=a[Math.floor(Math.random()*a.length)];c=d.x+b()*d.sigma,s=d.y+b()*d.sigma,l=d.hue}else{const d=Math.sqrt(Math.random())*i,g=Math.random()*u;c=o+Math.cos(g)*d+(Math.random()-.5)*20,s=o+Math.sin(g)*d+(Math.random()-.5)*20,l=200+Math.random()*40}_(e,c,s,l,.58,.22,.7+Math.random()*1.7)}y(e,208,.45,.2,o*.5),v(e,8)}function oe(e){p(e,{majorRadius:o*.5,axisRatio:.7+Math.random()*.3,rotation:Math.random()*u,centerHue:26,outerHue:286,concentration:1.05,irregularity:.3,dustStrength:.08,armCount:1});for(let n=0;n<300;n++){const i=Math.pow(Math.random(),.5)*o*.6,t=Math.random()*u,a=o+Math.cos(t)*i,r=o+Math.sin(t)*i;_(e,a,r,210,.42,.12,.5+Math.random()*1.3)}v(e,5)}const ie={spiral:J,barred:ee,elliptical:ne,lenticular:ae,irregular:te,unknown:oe},A=4,re=2,G=["spiral","barred","elliptical","lenticular","irregular","unknown"];function le(e){const n=G.indexOf(e);return n>=0?n:5}function de(){const e=A*h,n=re*h,i=new OffscreenCanvas(e,n),t=i.getContext("2d",{alpha:!0});t.clearRect(0,0,e,n);for(let r=0;r<G.length;r++){const c=G[r],s=r%A,l=Math.floor(r/A),d=new OffscreenCanvas(h,h),g=d.getContext("2d",{alpha:!0});g.clearRect(0,0,h,h);const x=g.createRadialGradient(o,o,0,o,o,o*.9);x.addColorStop(0,"rgba(20, 20, 30, 0.2)"),x.addColorStop(1,"rgba(0, 0, 0, 0)"),g.fillStyle=x,g.fillRect(0,0,h,h),ie[c](g),t.drawImage(d,s*h,l*h)}const a=new j(i);return a.generateMipmaps=!0,a.magFilter=Z,a.minFilter=Y,"colorSpace"in a&&(a.colorSpace=K),a.needsUpdate=!0,a}const ge=`// Existing attributes
...
`;export{de as a,ue as f,ge as g,le as m,ce as n,he as r};
