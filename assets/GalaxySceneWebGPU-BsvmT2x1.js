import{U as G,X as ct,Y,Z as e,_ as I,$ as D,a0 as W,a1 as U,a2 as it,a3 as rt,a4 as Kt,a5 as Mt,a6 as m,a7 as kt,a8 as ht,a9 as lt,aa as x,V as gt,ab as Et,ac as $,ad as _,ae as at,af as fe,ag as It,ah as tt,ai as Xt,aj as we,ak as ne,A as ae,u as ie,al as ce,k as Qt,am as ze,an as te,ao as ke,ap as Me,aq as Te,d as ve,ar as oe,M as le,as as Re,at as ue,au as De,av as he,w as Pe,l as Ce,aw as Be,ax as Le,ay as Ee,Q as $t,az as Fe,aA as Ie,f as _e,n as Yt,S as ee,P as He,aB as We}from"./three-BUZvkdOE.js";import{m as me}from"./morphologyMapper-DY_wjh1a.js";import{a as be,l as Ne,d as Ve}from"./nsaMorphologyPointCloud-Ct5fnwAD.js";import{G as xe}from"./index-D5ORmE3w.js";import{d as Ue,c as qe,a as Xe,g as Ye,b as Ge}from"./qualityDetect-DFgcI_oC.js";import"./vue-vendor-CyMGTBsb.js";const Oe=8,je=12,yt=.015;function Ze(n,t={}){const s=t.radialBinCount??Oe,i=t.azimuthalBinCount??je,o=Qe(t.availability),a=Array.from({length:s},()=>({intensity:0,hot:0,stellar:0,dust:0,weight:0})),l=Array.from({length:i},()=>({intensity:0,weight:0}));let c=0,d=0,p=0,w=0,S=0;const u=(n.width-1)*.5,h=(n.height-1)*.5,A=Math.max(1,Math.hypot(u,h));for(let z=0;z<n.height;z+=1)for(let H=0;H<n.width;H+=1){const X=z*n.width+H,E=re(n,X,o);if(E.intensity<=yt)continue;const O=H-u,N=z-h,V=K(Math.hypot(O,N)/A),Z=Math.atan2(N,O);c+=E.hot,d+=E.stellar,p+=E.dust,V<=.22&&(w+=E.intensity),V<=.85&&(S+=E.intensity);const et=Math.min(s-1,Math.floor(V*s)),P=a[et];if(P.intensity+=E.intensity,P.hot+=E.hot,P.stellar+=E.stellar,P.dust+=E.dust,P.weight+=1,V>=.2&&V<=.88){const k=Z<0?Z+Math.PI*2:Z,B=Math.min(i-1,Math.floor(k/(Math.PI*2)*i)),C=l[B];C.intensity+=E.intensity,C.weight+=1}}const b=Ke(c,d,p),r=a.map((z,H)=>Je(z,H,s)),f=l.map((z,H)=>({angle:H/i*Math.PI*2,intensity:z.weight>0?z.intensity/z.weight:0})),v=S>0?K(w/S):0,M=ts(n),y=ss(f.map(z=>z.intensity)),R=es(n),q=as(n,o),F=K((1-M)*.7+v*.3),Q=K(b.dust*(.55+M*.35+y*.25));return{availability:o,globalColorBalance:b,concentration:v,armContrast:y,clumpiness:R,filamentarity:M,diskThicknessBias:F,dustLaneStrength:Q,projectedAxisRatio:q.axisRatio,projectedAngle:q.angle,projectedStrength:q.strength,radialProfile:r,azimuthalProfile:f}}function Qe(n){const t=["u","g","r","i","z","nuv"],s={u:!0,g:!0,r:!0,i:!0,z:!0,nuv:!0},i={u:!1,g:!1,r:!1,i:!1,z:!1,nuv:!1};for(const o of t)(n==null?void 0:n[o])==="fallback"&&(s[o]=!1,i[o]=!0);return{real:s,fallback:i}}function re(n,t,s){const i=Math.sqrt(K(n.bands.u[t]??0)),o=Math.sqrt(K(n.bands.g[t]??0)),a=Math.sqrt(K(n.bands.r[t]??0)),l=Math.sqrt(K(n.bands.i[t]??0)),c=Math.sqrt(K(n.bands.z[t]??0)),d=Math.sqrt(K(n.bands.nuv[t]??0)),p=s.real.u?1:0,w=s.real.i?1:0,S=s.real.z?1:0,u=s.real.nuv?1:0,h=p+u,A=h>0?(i*p+d*u)/h:$e(o,a),b=(o+a)*.5,r=w+S,f=r>0?(l*w+c*S)/r:0,v=f*.25+b*.4+A*.35;return{hot:A,stellar:b,dust:f,intensity:v}}function $e(n,t){return(n+t)*.5*.35}function Ke(n,t,s){const i=n+t+s;return i<=0?{hot:0,stellar:0,dust:0}:{hot:n/i,stellar:t/i,dust:s/i}}function Je(n,t,s){return n.weight<=0?{radius:(t+.5)/s,intensity:0,hot:0,stellar:0,dust:0}:{radius:(t+.5)/s,intensity:n.intensity/n.weight,hot:n.hot/n.weight,stellar:n.stellar/n.weight,dust:n.dust/n.weight}}function ts(n){const t=Math.max(1,Math.floor(Math.max(n.width,n.height)/64)),{field:s,w:i,h:o}=be(n,t),a=Math.max(1,Math.min(3,Math.floor(Math.min(i,o)/12)));let l=0,c=0;for(let w=0;w<o;w+=1)for(let S=0;S<i;S+=1){const u=s[w*i+S];if(u<=yt)continue;const{F:h}=Ne(s,i,o,S,w,a);l+=h*u,c+=u}const d=c>0?K(l/c):0,p=ns(s,i,o);return K(d*.35+p*.65)}function es(n){const t=Math.max(1,Math.floor(Math.max(n.width,n.height)/96)),{field:s,w:i,h:o}=be(n,t);let a=0,l=0;for(let c=0;c<o;c+=1)for(let d=0;d<i;d+=1){const p=s[c*i+d];if(p<=yt)continue;let w=0,S=0;for(let h=-1;h<=1;h+=1)for(let A=-1;A<=1;A+=1){if(A===0&&h===0)continue;const b=Math.max(0,Math.min(i-1,d+A)),r=Math.max(0,Math.min(o-1,c+h));w+=s[r*i+b],S+=1}const u=S>0?w/S:p;a+=Math.max(p-u,0),l+=p}return l>0?K(a/l*3):0}function ss(n){const t=n.filter(o=>o>0);if(t.length===0)return 0;const s=t.reduce((o,a)=>o+a,0)/t.length;if(s<=1e-6)return 0;const i=t.reduce((o,a)=>{const l=a-s;return o+l*l},0)/t.length;return K(Math.sqrt(i)/s/2)}function ns(n,t,s){let i=0,o=0,a=0;for(let A=0;A<s;A+=1)for(let b=0;b<t;b+=1){const r=n[A*t+b];r<=yt||(i+=r,o+=b*r,a+=A*r)}if(i<=0)return 0;o/=i,a/=i;let l=0,c=0,d=0;for(let A=0;A<s;A+=1)for(let b=0;b<t;b+=1){const r=n[A*t+b];if(r<=yt)continue;const f=b-o,v=A-a;l+=f*f*r,c+=f*v*r,d+=v*v*r}l/=i,c/=i,d/=i;const p=l+d;if(p<=1e-6)return 0;const w=l*d-c*c,S=Math.sqrt(Math.max(0,p*p-4*w)),u=(p+S)*.5,h=(p-S)*.5;return K((u-h)/(u+h+1e-6))}function as(n,t){let s=0,i=0,o=0;for(let b=0;b<n.height;b+=1)for(let r=0;r<n.width;r+=1){const f=b*n.width+r,v=re(n,f,t).intensity;v<=yt||(s+=v,i+=r*v,o+=b*v)}if(s<=0)return{axisRatio:1,angle:0,strength:0};i/=s,o/=s;let a=0,l=0,c=0;for(let b=0;b<n.height;b+=1)for(let r=0;r<n.width;r+=1){const f=b*n.width+r,v=re(n,f,t).intensity;if(v<=yt)continue;const M=r-i,y=b-o;a+=M*M*v,l+=M*y*v,c+=y*y*v}a/=s,l/=s,c/=s;const d=a+c;if(d<=1e-6)return{axisRatio:1,angle:0,strength:0};const p=a*c-l*l,w=Math.sqrt(Math.max(0,d*d-4*p)),S=Math.max((d+w)*.5,1e-6),u=Math.max((d-w)*.5,1e-6),h=K(Math.sqrt(u/S)),A=.5*Math.atan2(2*l,a-c);return{axisRatio:Math.max(h,.15),angle:is(A),strength:K((1-h)*1.35)}}function K(n){return Math.max(0,Math.min(1,n))}function is(n){const t=Math.PI,s=n%t;return s<0?s+t:s}async function os(n){const t=await ls(n);if(!t)return null;const s=await rs(n,t);if(!(s!=null&&s.g)||!s.r||!s.i)return null;ds(s);const{input:i,availability:o}=us(s),a=Ze(i,{availability:hs(o)});return{metadata:t,input:i,availability:o,profile:a}}async function ls(n){try{const t=await fetch(`${xe}/${n}/metadata.json`);return t.ok?await t.json():null}catch{return null}}async function rs(n,t){const s=cs(t),i=`${xe}/${n}/`,o=await Promise.all(s.map(async a=>{try{const l=await fetch(`${i}${a}.png`);if(!l.ok){if(a==="g"||a==="r"||a==="i")throw new Error(`Missing required NSA band: ${a}`);return[a,null]}const c=await l.arrayBuffer(),d=Ve(new Uint8Array(c)),p=d.depth===16?65535:255,w=new Float32Array(d.width*d.height);for(let S=0;S<w.length;S+=1)w[S]=d.data[S*d.channels]/p;return[a,{raw:w,width:d.width,height:d.height,range:t.data_ranges[a]??[0,1]}]}catch(l){if(a==="g"||a==="r"||a==="i")throw l;return[a,null]}})).catch(()=>null);return o?Object.fromEntries(o.filter(a=>a[1]!==null)):null}function cs(n){const t=["i","r","g"];return n.bands.includes("u")&&t.push("u"),n.bands.includes("z")&&t.push("z"),n.bands.includes("nuv")&&t.push("nuv"),t}function ds(n){const t=n.g;if(!t)return;const s=t.range[1]-t.range[0];for(const i of["u","z","nuv"]){const o=n[i];if(!o)continue;o.range[1]-o.range[0]<s*.01&&delete n[i]}}function us(n){const t=a=>n[a]?a:a==="u"?"g":a==="z"?"i":n.u?"u":"g",s=n.i??n.g??n.r;if(!s)throw new Error("Cannot build point-cloud input without core NSA bands");const i={u:t("u"),g:t("g"),r:t("r"),i:t("i"),z:t("z"),nuv:t("nuv")},o={real:{u:i.u==="u",g:i.g==="g",r:i.r==="r",i:i.i==="i",z:i.z==="z",nuv:i.nuv==="nuv"},fallback:{u:i.u!=="u",g:!1,r:!1,i:!1,z:i.z!=="z",nuv:i.nuv!=="nuv"}};return{input:{width:s.width,height:s.height,bands:{u:zt(n[i.u]),g:zt(n[i.g]),r:zt(n[i.r]),i:zt(n[i.i]),z:zt(n[i.z]),nuv:zt(n[i.nuv])}},availability:o}}function zt(n){const[t,s]=n.range,i=s-t,o=new Float32Array(n.raw.length);let a=0;for(let l=0;l<n.raw.length;l+=1){const c=n.raw[l]*i+t,d=Math.max(c,0);o[l]=d,d>a&&(a=d)}if(a>0)for(let l=0;l<o.length;l+=1)o[l]/=a;return o}function hs(n){return{u:n.fallback.u?"fallback":"real",g:"real",r:"real",i:"real",z:n.fallback.z?"fallback":"real",nuv:n.fallback.nuv?"fallback":"real"}}const g=G(([n])=>{const t=ct(n.mul(.1031)),s=t.add(19.19);return ct(s.mul(s.add(47.43)).mul(t))}),ut=G(([n])=>{const t=ct(n.mul(D(.16532,.17369,.15787))).toVar();return t.addAssign(Kt(t,t.yzx.add(19.19))),ct(t.x.mul(t.y).mul(t.z))}),Gt=G(([n])=>ct(W(Kt(n,kt(2.31,53.21)).mul(124.123)).mul(412)));G(([n])=>{const t=Mt(n),s=ct(n),i=s.mul(s).mul(e(3).sub(s.mul(2)));return m(m(Gt(t),Gt(t.add(kt(1,0))),i.x),m(Gt(t.add(kt(0,1))),Gt(t.add(kt(1,1))),i.x),i.y)});const se=G(([n])=>{const t=Mt(n),s=ct(n),i=s.mul(s).mul(e(3).sub(s.mul(2))),o=ut(t.add(D(0,0,0))),a=ut(t.add(D(1,0,0))),l=ut(t.add(D(0,1,0))),c=ut(t.add(D(1,1,0))),d=ut(t.add(D(0,0,1))),p=ut(t.add(D(1,0,1))),w=ut(t.add(D(0,1,1))),S=ut(t.add(D(1,1,1))),u=m(o,a,i.x),h=m(l,c,i.x),A=m(d,p,i.x),b=m(w,S,i.x),r=m(u,h,i.y),f=m(A,b,i.y);return m(r,f,i.z).mul(2).sub(1)});G(([n,t])=>it(n).sub(t));G(([n,t])=>{const s=kt(it(n.xz).sub(t.x),n.y);return it(s).sub(t.y)});const Ft=G(([n,t])=>{const s=W(t),i=U(t),o=n.x.mul(s).sub(n.z.mul(i)),a=n.x.mul(i).add(n.z.mul(s));return D(o,n.y,a)}),pt=G(([n,t,s])=>{const i=it(D(n.x,e(0),n.z)),o=e(20),a=I(i,o).div(o),l=e(1).div(rt(a,e(.35))),c=t.mul(l).mul(s).negate();return Ft(n,c)});G(([n,t,s,i,o,a])=>{const l=t.sub(n),c=it(l),d=s.mul(I(e(0),e(1).sub(c.div(o))));return ht(l).mul(i).mul(d).mul(a).negate()});G(([n,t,s,i])=>t.sub(n).mul(s).mul(i));const ms=G(([n,t,s])=>{const i=t.mul(Y(s,e(1).sub(s))),o=ct(n.mul(12).add(0).div(12)).mul(12),a=ct(n.mul(12).add(8).div(12)).mul(12),l=ct(n.mul(12).add(4).div(12)).mul(12),c=s.sub(i.mul(I(Y(Y(o.sub(3),e(9).sub(o)),e(1)),e(-1)))),d=s.sub(i.mul(I(Y(Y(a.sub(3),e(9).sub(a)),e(1)),e(-1)))),p=s.sub(i.mul(I(Y(Y(l.sub(3),e(9).sub(l)),e(1)),e(-1))));return D(c,d,p)});function gs(n){return{positionBuffer:lt(n,"vec3"),originalPositionBuffer:lt(n,"vec3"),velocityBuffer:lt(n,"vec3"),colorBuffer:lt(n,"vec4"),sizeBuffer:lt(n,"float"),layerBuffer:lt(n,"float"),foregroundAlphaBuffer:lt(n,"float")}}function ps(n){const t={numArms:x(0),armWidth:x(0),spiralTightness:x(0),spiralStart:x(0),bulgeRadius:x(0),fieldStarFraction:x(0),irregularity:x(0),barLength:x(0),barWidth:x(0),axisRatio:x(1),ellipticity:x(0),bulgeFraction:x(0),diskThickness:x(0),clumpCount:x(0),galaxyRadius:x(0),galaxySeed:x(0),bandArmScatterScale:x(1),bandBulgeBoost:x(0),bandClumpBoost:x(0),bandHotMix:x(.5),bandDustMix:x(.5),bandDiskThicknessScale:x(1),bandDustLaneStrength:x(0),coreWeight:x(1),midDiskWeight:x(1),outerDiskWeight:x(1),peakAzimuthAngleA:x(0),peakAzimuthAngleB:x(Math.PI),peakAzimuthStrength:x(0),projectedAxisRatio:x(1),projectedAngle:x(0),projectedStrength:x(0),time:x(0),deltaTime:x(.016),rotationSpeed:x(.033),mouse:x(new gt(0,0,0)),mouseActive:x(0),mouseForce:x(7),mouseRadius:x(n.galaxyRadius*.3)};return ye(t,n),t}function ye(n,t){const s=t.morphology,i=Ue(t.bandProfile);n.numArms.value=s.numArms,n.armWidth.value=s.armWidth*t.galaxyRadius,n.spiralTightness.value=s.spiralTightness,n.spiralStart.value=s.spiralStart,n.bulgeRadius.value=s.bulgeRadius*t.galaxyRadius,n.fieldStarFraction.value=s.fieldStarFraction,n.irregularity.value=s.irregularity,n.barLength.value=s.barLength*t.galaxyRadius,n.barWidth.value=s.barWidth*t.galaxyRadius,n.axisRatio.value=s.axisRatio,n.ellipticity.value=s.ellipticity,n.bulgeFraction.value=s.bulgeFraction,n.diskThickness.value=s.diskThickness,n.clumpCount.value=s.clumpCount,n.galaxyRadius.value=t.galaxyRadius,n.galaxySeed.value=t.starCount*.61803398875,n.bandArmScatterScale.value=i.armScatterScale,n.bandBulgeBoost.value=i.bulgeBoost,n.bandClumpBoost.value=i.clumpBoost,n.bandHotMix.value=i.hotMix,n.bandDustMix.value=i.dustMix,n.bandDiskThicknessScale.value=i.diskThicknessScale,n.bandDustLaneStrength.value=i.dustLaneStrength,n.coreWeight.value=i.coreWeight,n.midDiskWeight.value=i.midDiskWeight,n.outerDiskWeight.value=i.outerDiskWeight,n.peakAzimuthAngleA.value=i.peakAzimuthAngleA,n.peakAzimuthAngleB.value=i.peakAzimuthAngleB,n.peakAzimuthStrength.value=i.peakAzimuthStrength,n.projectedAxisRatio.value=i.projectedAxisRatio,n.projectedAngle.value=i.projectedAngle,n.projectedStrength.value=i.projectedStrength,n.mouseRadius.value=t.galaxyRadius*.3}const xt=6.28318530718;function Lt(n,t){const s=tt(e(.18),e(.38),n),i=tt(e(.58),e(.82),n),o=m(t.coreWeight,t.midDiskWeight,s);return m(o,t.outerDiskWeight,i)}function fs(n,t){const s=W(t.projectedAngle),i=U(t.projectedAngle),o=n.x.mul(s).add(n.z.mul(i)),a=n.z.mul(s).sub(n.x.mul(i)),l=m(e(1),t.projectedAxisRatio,t.projectedStrength),c=a.mul(l);return D(o.mul(s).sub(c.mul(i)),n.y,o.mul(i).add(c.mul(s)))}function vs(n,t,s){return G(()=>{const o=Et,a=o.toFloat(),l=s.galaxyRadius,c=l.mul(.06),d=m(e(.55),e(.76),$(s.bandDustMix.add(s.bandDustLaneStrength.mul(.35)),e(0),e(1))),p=m(e(.02),e(.08),$(s.bandHotMix.mul(.7).add(s.bandClumpBoost.mul(.3)),e(0),e(1))),w=g(a.add(100)),S=e(0).toVar();_(w.greaterThan(e(1).sub(p)),()=>{S.assign(2)}).ElseIf(w.greaterThan(d),()=>{S.assign(1)}),t.layerBuffer.element(o).assign(S);const u=g(a.add(200)),h=e(0).toVar();_(S.equal(0),()=>{h.assign(u.mul(1.5).add(.8).mul(m(e(.95),e(1.35),s.bandDustMix)))}).ElseIf(S.equal(1),()=>{h.assign(u.mul(3).add(1.5))}).Else(()=>{h.assign(u.mul(6).add(4).mul(m(e(.9),e(1.4),s.bandHotMix)))}),t.sizeBuffer.element(o).assign(h);const A=g(a.add(300)),b=g(a.add(400)),r=e(0).toVar(),f=e(0).toVar();_(S.equal(0),()=>{r.assign(A.mul(.16).add(.08).mul(m(e(.9),e(1.35),s.bandDustMix))),f.assign(b.mul(.2).add(.12).mul(m(e(.9),e(1.2),s.bandDustLaneStrength)))}).ElseIf(S.equal(1),()=>{r.assign(A.mul(.4).add(.32).mul(m(e(.95),e(1.15),s.bandHotMix))),f.assign(b.mul(.4).add(.4))}).Else(()=>{r.assign(A.mul(.16).add(.64).mul(m(e(.95),e(1.25),s.bandHotMix))),f.assign(b.mul(.24).add(.56).mul(m(e(.95),e(1.2),s.bandClumpBoost)))});const v=e(0).toVar(),M=e(0).toVar(),y=e(0).toVar(),R=e(0).toVar(),q=e(-1).toVar(),F=e(1).toVar(),Q=e(0).toVar();_(s.numArms.greaterThan(0),()=>{const P=g(a.add(500)),k=s.bulgeRadius,B=Y(e(.25),e(.1).add(e(.2).mul(k.div(l)))),C=s.fieldStarFraction;_(P.lessThan(B),()=>{q.assign(0);const T=rt(g(a.add(10)),e(.6)).mul(k),L=g(a.add(11)).mul(xt),j=g(a.add(12)).sub(.5).mul(k).mul(.5);v.assign(W(L).mul(T)),M.assign(j),y.assign(U(L).mul(T)),R.assign(T.div(k).mul(.3)),F.assign(s.coreWeight)}).ElseIf(P.lessThan(B.add(C)),()=>{q.assign(1);const T=at(g(a.add(20))).mul(l),L=g(a.add(21)).mul(xt),j=g(a.add(22)).sub(.5).mul(l).mul(.08);v.assign(W(L).mul(T)),M.assign(j),y.assign(U(L).mul(T)),R.assign(T.div(l)),F.assign(Lt(R,s))}).Else(()=>{q.assign(2);const T=s.numArms,j=Mt(g(a.add(30)).mul(T)).mul(xt).div(T),J=I(s.spiralStart.mul(l),e(.001)),ot=s.barLength.mul(.5),st=Y(I(J,ot),l.mul(.98)),nt=g(a.add(31)),mt=at(nt.mul(l.mul(l).sub(st.mul(st))).add(st.mul(st))),ft=mt.div(l),vt=Lt(ft,s);F.assign(vt);const Tt=e(2.5),St=I(mt.div(J),e(1)).log().div(I(s.spiralTightness,e(.001))).mul(Tt),bt=mt.div(l).mul(.5).add(.5),At=g(a.add(32)).sub(.5).add(g(a.add(33)).sub(.5)).mul(s.armWidth).mul(bt).mul(s.bandArmScatterScale),Rt=s.irregularity.mul(g(a.add(35)).sub(.5)).mul(30),_t=g(a.add(34)).sub(.5).mul(.3),dt=St.add(j).add(_t),Ht=W(dt.sub(s.peakAzimuthAngleA)).mul(.5).add(.5),Wt=W(dt.sub(s.peakAzimuthAngleB)).mul(.5).add(.5),Nt=I(Ht,Wt);Q.assign(Nt);const Vt=U(s.peakAzimuthAngleA.sub(dt)).mul(Ht).add(U(s.peakAzimuthAngleB.sub(dt)).mul(Wt)).mul(s.peakAzimuthStrength).mul(.22),Dt=dt.add(Vt),Ut=Dt.add(e(Math.PI/2)),qt=m(e(.86),e(1.18),$(vt.sub(.55).div(1.3),e(0),e(1))),Pt=$(mt.mul(qt),st,l),Ct=At.mul(m(e(1.3),e(.42),Nt.mul(s.peakAzimuthStrength))),Bt=W(Dt).mul(Pt.add(Rt)).add(W(Ut).mul(Ct)),wt=U(Dt).mul(Pt.add(Rt)).add(U(Ut).mul(Ct)),de=Pt.div(l),Jt=l.mul(.06).mul(e(1).sub(de.mul(.7))).mul(s.bandDiskThicknessScale),Se=g(a.add(36)).sub(.5).mul(Jt);v.assign(Bt),M.assign(Se),y.assign(wt);const Ae=at(Bt.mul(Bt).add(wt.mul(wt)));R.assign(Ae.div(l))}),_(s.barLength.greaterThan(0),()=>{const T=g(a.add(600));_(T.lessThan(.25),()=>{const L=s.barLength,j=s.barWidth,J=g(a.add(40)).sub(.5).mul(2).mul(L),ot=g(a.add(41)).sub(.5).mul(j);v.assign(J),M.assign(g(a.add(42)).sub(.5).mul(l).mul(.04)),y.assign(ot),R.assign(e(.1))})})}),_(s.numArms.equal(0).and(s.barLength.equal(0)).and(s.clumpCount.equal(0)).and(s.ellipticity.equal(0)).and(s.bulgeFraction.greaterThan(0)),()=>{const P=s.bulgeRadius,k=rt(g(a.add(10)),e(.55)).mul(l),B=g(a.add(11)).mul(xt),C=k.div(l),T=l.mul(.06).mul(rt(I(e(1).sub(C),e(0)),e(2))).mul(s.bandDiskThicknessScale),L=g(a.add(12)).sub(.5).mul(T);v.assign(W(B).mul(k)),M.assign(L),y.assign(U(B).mul(k));const j=$(e(1).sub(k.div(I(P,e(1)))),e(0),e(1)),J=e(1).add(j.mul(.4)).add(s.bandBulgeBoost.mul(.25));r.assign(Y(r.mul(J),e(.95))),f.assign(Y(f.mul(J),e(.95))),h.assign(h.mul(e(1).add(j.mul(.3)))),R.assign(C.mul(.2)),F.assign(Lt(C,s))}),_(s.ellipticity.greaterThan(0),()=>{const P=s.axisRatio,k=rt(g(a.add(10)),e(.4)).mul(l),B=g(a.add(11)).mul(xt),C=k.mul(W(B)),T=k.mul(U(B)).mul(P),L=at(C.mul(C).add(T.mul(T))).div(l),j=g(a.add(12)).sub(.5).mul(l).mul(.1).mul(e(1).sub(L.mul(.5)));v.assign(C),M.assign(j),y.assign(T),R.assign(L),F.assign(Lt(L,s))}),_(s.clumpCount.greaterThan(0),()=>{const P=s.irregularity,k=s.clumpCount,B=g(a.add(500));_(B.greaterThan(P),()=>{const C=Mt(g(a.add(50)).mul(k)),T=C.div(k).mul(xt).add(g(C.add(1e3)).mul(.5)),L=g(C.add(2e3)).mul(.6).add(.2).mul(l),j=W(T).mul(L),J=U(T).mul(L),ot=g(C.add(3e3)).mul(80).add(30).mul(m(e(1.05),e(.7),s.bandClumpBoost)),st=g(a.add(51)).sub(.5).add(g(a.add(52)).sub(.5)).mul(2),nt=g(a.add(53)).sub(.5).add(g(a.add(54)).sub(.5)).mul(2);v.assign(j.add(st.mul(ot))),y.assign(J.add(nt.mul(ot)))}).Else(()=>{const C=g(a.add(60)).mul(xt),T=at(g(a.add(61))).mul(l);v.assign(W(C).mul(T).add(g(a.add(62)).sub(.5).mul(60))),y.assign(U(C).mul(T).add(g(a.add(63)).sub(.5).mul(60)))}),M.assign(g(a.add(70)).sub(.5).mul(l).mul(.12)),R.assign(at(v.mul(v).add(y.mul(y))).div(l)),F.assign(Lt(R,s))}),_(q.equal(0),()=>{const P=m(e(1),s.coreWeight.mul(.55).add(.45),s.bandBulgeBoost);r.assign(Y(r.mul(P),e(.98))),f.assign(Y(f.mul(P),e(.98))),h.assign(h.mul(m(e(1),e(1.35),s.bandBulgeBoost)))}).ElseIf(q.equal(1),()=>{const P=m(e(1),e(.62),s.peakAzimuthStrength);r.assign(r.mul(P)),f.assign(f.mul(m(e(1),e(.8),s.peakAzimuthStrength)))}).ElseIf(q.equal(2),()=>{const P=m(e(.72),e(1.48),Q.mul(s.peakAzimuthStrength)),k=m(e(.78),e(1.34),$(F.sub(.55).div(1.3),e(0),e(1)));r.assign(Y(r.mul(P).mul(k),e(.98))),f.assign(Y(f.mul(P),e(.98))),h.assign(h.mul(m(e(.92),e(1.4),Q.mul(s.peakAzimuthStrength))))});const z=fs(D(v,M,y),s);v.assign(z.x),M.assign(z.y),y.assign(z.z),R.assign(Y(at(v.mul(v).add(y.mul(y))).div(l),e(1)));const H=at(v.mul(v).add(y.mul(y)));_(H.lessThan(c),()=>{const P=fe(y,v),k=c.add(g(a.add(800)).mul(l.mul(.1)));v.assign(W(P).mul(k)),y.assign(U(P).mul(k))});const X=D(v,M,y);t.positionBuffer.element(o).assign(X),t.originalPositionBuffer.element(o).assign(X);const E=g(a.add(900)),O=g(a.add(901)),N=e(0).toVar(),V=e(0).toVar(),Z=e(0).toVar();_(S.equal(0),()=>{N.assign(E.mul(.111).add(.667)),V.assign(.3),Z.assign(r.mul(.4))}).ElseIf(S.equal(2),()=>{_(E.lessThan(e(.6)),()=>{N.assign(E.div(.6).mul(.097).add(.028)),V.assign(.5)}).Else(()=>{N.assign(E.sub(.6).div(.4).mul(.083).add(.556)),V.assign(.35)}),Z.assign(r.mul(.85))}).Else(()=>{const P=rt($(R,e(0),e(1)),e(.6)),k=s.bandHotMix.sub(s.bandDustMix).mul(.16),B=$(m(e(.58),e(.3),P).sub(k),e(.12),e(.8)),C=$(m(e(.78),e(.45),P).sub(k.mul(.75)),e(.22),e(.9)),T=$(m(e(.88),e(.55),P).sub(k.mul(.45)),e(.35),e(.96)),L=$(m(e(.93),e(.65),P).sub(k.mul(.25)),e(.45),e(.98)),j=$(m(e(.98),e(.87),P).sub(k.mul(.1)),e(.7),e(.995));_(E.lessThan(B),()=>{N.assign(e(.028).add(O.sub(.5).mul(.022))),V.assign(.85)}).ElseIf(E.lessThan(C),()=>{N.assign(e(.069).add(O.sub(.5).mul(.022))),V.assign(.6)}).ElseIf(E.lessThan(T),()=>{N.assign(e(.133).add(O.sub(.5).mul(.014))),V.assign(.22)}).ElseIf(E.lessThan(L),()=>{N.assign(e(.153).add(O.sub(.5).mul(.011))),V.assign(.12)}).ElseIf(E.lessThan(j),()=>{N.assign(e(.597).add(O.sub(.5).mul(.042))),V.assign(.25)}).Else(()=>{N.assign(e(.625).add(O.sub(.5).mul(.028))),V.assign(.45)}),Z.assign(r.mul(.6))});const et=ms(N,V,Z);t.colorBuffer.element(o).assign(It(et.x,et.y,et.z,f)),t.velocityBuffer.element(o).assign(D(0,0,0))})().compute(n)}function bs(n,t,s){return G(()=>{const o=Et,a=t.positionBuffer.element(o).toVar(),l=t.originalPositionBuffer.element(o);_(s.barLength.greaterThan(0),()=>{const d=it(D(a.x,e(0),a.z)),p=s.rotationSpeed.mul(s.deltaTime).negate();_(d.lessThan(s.barLength),()=>{a.assign(Ft(a,p)),t.originalPositionBuffer.element(o).assign(Ft(l,p))}).Else(()=>{a.assign(pt(a,s.rotationSpeed,s.deltaTime)),t.originalPositionBuffer.element(o).assign(pt(l,s.rotationSpeed,s.deltaTime))})}).Else(()=>{const d=pt(a,s.rotationSpeed,s.deltaTime);a.assign(d),t.originalPositionBuffer.element(o).assign(pt(l,s.rotationSpeed,s.deltaTime))}),t.positionBuffer.element(o).assign(a);const c=t.layerBuffer.element(o);_(c.equal(2),()=>{const d=t.colorBuffer.element(o),p=o.toFloat().mul(.7831),w=U(s.time.mul(2).add(p)).mul(.15).add(.85),S=d.w;t.colorBuffer.element(o).w.assign(S.mul(w))})})().compute(n)}function xs(){return{mvpRow0:x(new Xt),mvpRow1:x(new Xt),mvpRow3:x(new Xt),viewZRow:x(new Xt),bhViewZ:x(0),bhNdcX:x(0),bhNdcY:x(0),ndcRadiusX:x(.04),ndcRadiusY:x(.04),depthThreshold:x(6),depthSoftness:x(10)}}function ys(n,t,s){return G(()=>{const o=Et,a=t.positionBuffer.element(o),l=s.mvpRow0,c=s.mvpRow1,d=s.mvpRow3,p=s.viewZRow,w=l.x.mul(a.x).add(l.y.mul(a.y)).add(l.z.mul(a.z)).add(l.w),S=c.x.mul(a.x).add(c.y.mul(a.y)).add(c.z.mul(a.z)).add(c.w),u=d.x.mul(a.x).add(d.y.mul(a.y)).add(d.z.mul(a.z)).add(d.w),h=p.x.mul(a.x).add(p.y.mul(a.y)).add(p.z.mul(a.z)).add(p.w),A=e(1).div(I(u,e(1e-4))),b=w.mul(A),r=S.mul(A),f=b.sub(s.bhNdcX).div(s.ndcRadiusX),v=r.sub(s.bhNdcY).div(s.ndcRadiusY),M=e(1).sub(tt(e(.75),e(1.25),at(f.mul(f).add(v.mul(v))))),y=tt(s.depthThreshold,s.depthThreshold.add(s.depthSoftness),h.sub(s.bhViewZ));t.foregroundAlphaBuffer.element(o).assign(M.mul(y))})().compute(n)}class Ss{constructor(t,s,i){const o=s.positionBuffer.toAttribute(),a=s.colorBuffer.toAttribute(),l=s.sizeBuffer.toAttribute(),c=s.foregroundAlphaBuffer.toAttribute(),d=Math.sqrt(6e4/t),p=i*.003*d,w=qe();this.glowTexture=w;const S=we(w),u=h=>{const A=S.sample(ce()),b=a.w.mul(A.w).mul(h),r=m(D(a.x,a.y,a.z).mul(1.32),D(1,1,1),e(.12)),f=m(r,D(1,1,1),e(.92)),v=r.mul(A.x).add(f.mul(A.y.mul(1.35)));return It(v.mul(b),b)};this.material=new ne,this.material.transparent=!0,this.material.depthWrite=!1,this.material.blending=ae,this.material.positionNode=o,this.material.scaleNode=l.mul(p),this.material.colorNode=u(e(1).sub(c)),this.sprite=new ie(this.material),this.sprite.count=t,this.sprite.frustumCulled=!1,this.foregroundMaterial=new ne,this.foregroundMaterial.transparent=!0,this.foregroundMaterial.depthWrite=!1,this.foregroundMaterial.blending=ae,this.foregroundMaterial.positionNode=o,this.foregroundMaterial.scaleNode=l.mul(p),this.foregroundMaterial.colorNode=u(c),this.foregroundSprite=new ie(this.foregroundMaterial),this.foregroundSprite.count=t,this.foregroundSprite.frustumCulled=!1,this.foregroundSprite.renderOrder=2}dispose(){this.material.dispose(),this.foregroundMaterial.dispose(),this.glowTexture.dispose()}}const As=G(([n])=>{const t=I(n.r,I(n.g,n.b)),s=Y(n.r,Y(n.g,n.b)),i=t.sub(s),o=tt(e(.06),e(.3),i).mul(e(1).sub(tt(e(.28),e(.95),t)));return rt(I(n,D(0)),D(1.14,1.14,1.14)).mul(m(e(.9),e(.45),o)).mul(D(1.06,.93,.82))});class ws{constructor(t,s,i,o,a){this.uBHScreenPos=x(new Qt(.5,.5)),this.uLensStrength=x(0),this.uAspectRatio=x(1),this.postProcessing=new ze(t);const c=te(s,a).getTextureNode(),p=te(i,a).getTextureNode(),S=te(o,a).getTextureNode(),u=this.uBHScreenPos,h=this.uLensStrength,A=this.uAspectRatio,r=G(()=>{const M=ke.toVar(),y=u.sub(M).toVar();y.x.mulAssign(A);const R=it(y),q=y.div(I(R,e(1e-4))),F=$(h.div(.03),e(0),e(1)),Q=m(e(.25),e(.55),F),z=tt(Q,e(0),R).toVar();z.mulAssign(z);const H=m(e(.012),e(.05),F),X=tt(H,H.mul(2.8),R),E=I(R,m(e(.028),e(.04),F)),O=h.mul(z).mul(X).mul(m(e(.15),e(.3),F).div(E)),N=q.mul(O).toVar();N.x.divAssign(A);const V=$(M.add(N),e(0),e(1)),Z=c.sample(V).toVar();Z.rgb.assign(As(Z.rgb));const et=m(e(.024),e(.09),F),P=m(e(.008),e(.024),F),B=Me(rt(R.sub(et).div(P),e(2)).negate()).mul(z).mul(X).mul(h).mul(m(e(10),e(16),F));return Z.rgb.addAssign(D(.72,.62,.46).mul(B.mul(.02))),Z})();this.bloomPassNode=Te(c),this.bloomPassNode.threshold.value=.2,this.bloomPassNode.strength.value=.12,this.bloomPassNode.radius.value=.08;const f=r.add(this.bloomPassNode),v=G(()=>{const M=f,y=p,R=S,F=m(M.rgb,y.rgb.mul(D(1.05,.95,.84)),y.a).add(R.rgb),Q=I(M.a,I(y.a,R.a));return It(Y(F,e(1)),Q)});this.postProcessing.outputNode=v()}render(){this.postProcessing.render()}updateBloom(t,s,i){this.bloomPassNode.strength.value=t,this.bloomPassNode.radius.value=s,this.bloomPassNode.threshold.value=i}updateLensing(t,s,i){this.uBHScreenPos.value.copy(t),this.uLensStrength.value=s,this.uAspectRatio.value=i}dispose(){}}const Ot=6.28318530718,zs=3e4;function ks(n){return n==="mobile"?1e4:zs}function jt(n,t){const s=tt(e(.18),e(.38),n),i=tt(e(.58),e(.82),n),o=m(t.coreWeight,t.midDiskWeight,s);return m(o,t.outerDiskWeight,i)}function Ms(n,t){const s=W(n.sub(t.peakAzimuthAngleA)).mul(.5).add(.5),i=W(n.sub(t.peakAzimuthAngleB)).mul(.5).add(.5);return I(s,i)}function Ts(n,t){const s=W(t.projectedAngle),i=U(t.projectedAngle),o=n.x.mul(s).add(n.z.mul(i)),a=n.z.mul(s).sub(n.x.mul(i)),l=m(e(1),t.projectedAxisRatio,t.projectedStrength),c=a.mul(l);return D(o.mul(s).sub(c.mul(i)),n.y,o.mul(i).add(c.mul(s)))}class Rs{constructor(t,s,i){const o=ks(i);this.positionBuffer=lt(o,"vec3"),this.originalPositionBuffer=lt(o,"vec3"),this.colorBuffer=lt(o,"vec3"),this.sizeBuffer=lt(o,"float");const a=this.positionBuffer,l=this.originalPositionBuffer,c=this.colorBuffer,d=this.sizeBuffer;this.computeInit=G(()=>{const b=Et,r=b.toFloat().add(1e4),f=t.galaxyRadius,v=e(0).toVar(),M=e(0).toVar(),y=e(0).toVar(),R=e(0).toVar(),q=e(1).toVar(),F=e(0).toVar();_(t.numArms.greaterThan(0),()=>{const k=g(r.add(1)),B=I(t.spiralStart.mul(f),e(.001)),C=t.barLength.mul(.5),T=Y(I(B,C),f.mul(.98)),L=at(k.mul(f.mul(f).sub(T.mul(T))).add(T.mul(T)));R.assign(L.div(f)),q.assign(jt(R,t));const J=Mt(g(r.add(2)).mul(t.numArms)).mul(Ot).div(t.numArms),ot=e(2.5),st=I(L.div(B),e(1)).log().div(I(t.spiralTightness,e(.001))).mul(ot),nt=J.add(st),mt=Ms(nt,t);F.assign(mt);const ft=U(t.peakAzimuthAngleA.sub(nt)).mul(W(nt.sub(t.peakAzimuthAngleA)).mul(.5).add(.5)).add(U(t.peakAzimuthAngleB.sub(nt)).mul(W(nt.sub(t.peakAzimuthAngleB)).mul(.5).add(.5))).mul(t.peakAzimuthStrength).mul(.26),vt=nt.add(ft),Tt=g(r.add(3)).sub(.5).mul(m(e(.2),e(.035),F.mul(t.peakAzimuthStrength).add(t.bandDustLaneStrength).mul(.5))),St=g(r.add(4)).sub(.5).mul(t.armWidth).mul(m(e(.55),e(.12),F.mul(t.peakAzimuthStrength).add(t.bandDustLaneStrength).mul(.5))).mul(m(e(.85),e(1.18),Y(q,e(1.25)).sub(.25))),bt=vt.add(Tt);v.assign(W(bt).mul(L.add(St))),y.assign(U(bt).mul(L.add(St)));const At=e(1).sub(R).add(.15).mul(t.bandDiskThicknessScale);M.assign(g(r.add(5)).sub(.5).mul(f.mul(.03)).mul(At))}),_(t.numArms.equal(0).and(t.barLength.equal(0)).and(t.clumpCount.equal(0)).and(t.ellipticity.equal(0)).and(t.bulgeFraction.greaterThan(0)),()=>{const k=rt(g(r.add(1)),e(.5)).mul(f),B=g(r.add(2)).mul(Ot);R.assign(k.div(f)),q.assign(jt(R,t)),v.assign(W(B).mul(k)),y.assign(U(B).mul(k));const C=f.mul(.03).mul(e(1).sub(R.mul(.5))).mul(t.bandDiskThicknessScale);M.assign(g(r.add(5)).sub(.5).mul(C))}),_(t.ellipticity.greaterThan(0),()=>{const k=t.axisRatio,B=rt(g(r.add(1)),e(.4)).mul(f),C=g(r.add(2)).mul(Ot),T=B.mul(W(C)),L=B.mul(U(C)).mul(k);R.assign(at(T.mul(T).add(L.mul(L))).div(f)),q.assign(jt(R,t)),v.assign(T),y.assign(L),M.assign(g(r.add(5)).sub(.5).mul(f).mul(.08).mul(e(1).sub(R.mul(.5))))}),_(t.clumpCount.greaterThan(0),()=>{const k=t.clumpCount,B=Mt(g(r.add(2)).mul(k)),C=B.div(k).mul(Ot).add(g(B.add(5e3)).mul(.5)),T=g(B.add(6e3)).mul(.6).add(.2).mul(f),L=W(C).mul(T),j=U(C).mul(T),J=g(B.add(7e3)).mul(80).add(30),ot=g(r.add(3)).sub(.5).add(g(r.add(4)).sub(.5)).mul(2),st=g(r.add(7)).sub(.5).add(g(r.add(8)).sub(.5)).mul(2);v.assign(L.add(ot.mul(J))),y.assign(j.add(st.mul(J))),R.assign(at(v.mul(v).add(y.mul(y))).div(f)),q.assign(jt(R,t)),M.assign(g(r.add(5)).sub(.5).mul(f).mul(.1))});const Q=Ts(D(v,M,y),t);v.assign(Q.x),M.assign(Q.y),y.assign(Q.z),R.assign(Y(at(v.mul(v).add(y.mul(y))).div(f),e(1)));const z=D(v,M,y);a.element(b).assign(z),l.element(b).assign(z);const H=D(.62,.7,.9),X=D(.92,.76,.62),E=m(H,X,t.bandDustMix),O=m(e(.7),e(1),t.bandDustLaneStrength),N=m(e(.58),e(1.3),F.mul(t.peakAzimuthStrength)),V=m(e(.72),e(1.22),Y(q,e(1.3)).sub(.3)),Z=E.mul(e(.72).sub(R.mul(.28))).mul(O).mul(N).mul(V);c.element(b).assign(Z);const et=e(1).sub(R.mul(.5)).mul(m(e(.9),e(1.15),t.bandDustLaneStrength)),P=g(r.add(6)).mul(.5).add(.7).mul(et).mul(m(e(.8),e(1.35),F.mul(t.peakAzimuthStrength)));d.element(b).assign(P)})().compute(o),this.computeUpdate=G(()=>{const b=Et,r=a.element(b).toVar(),f=l.element(b);_(t.barLength.greaterThan(0),()=>{const v=it(D(r.x,e(0),r.z)),M=t.rotationSpeed.mul(t.deltaTime).negate();_(v.lessThan(t.barLength),()=>{r.assign(Ft(r,M)),l.element(b).assign(Ft(f,M))}).Else(()=>{r.assign(pt(r,t.rotationSpeed,t.deltaTime)),l.element(b).assign(pt(f,t.rotationSpeed,t.deltaTime))})}).Else(()=>{r.assign(pt(r,t.rotationSpeed,t.deltaTime)),l.element(b).assign(pt(f,t.rotationSpeed,t.deltaTime))}),a.element(b).assign(r)})().compute(o),this.material=new ne,this.material.transparent=!0,this.material.depthWrite=!1,this.material.blending=ae;const p=a.toAttribute(),w=c.toAttribute(),S=d.toAttribute();this.material.positionNode=p;const u=G(()=>{const b=ce().sub(.5).mul(2),r=it(b),f=tt(1,0,r).mul(tt(1,.3,r));return It(w.x,w.y,w.z,f.mul(.015))})();this.material.colorNode=u;const h=Math.sqrt(6e4/o),A=s*.006*h;this.material.scaleNode=S.mul(A),this.sprite=new ie(this.material),this.sprite.count=o,this.sprite.frustumCulled=!1,this.sprite.renderOrder=-1}dispose(){this.material.dispose()}}class Ds{constructor(t=60){this.uTime=x(0),this.uTiltX=x(0),this.uRotY=x(0),this.uLOD=x(0),this.quadSize=t;const s=new ve(1,4,4),i=new oe({visible:!1});this.depthMesh=new le(s,i);const o=this.uTime,a=this.uTiltX,l=this.uRotY,c=this.uLOD,d=G(()=>{const w=ce().sub(.5).mul(2),S=it(w);_(S.greaterThan(1),()=>{Re()});const u=D(0,-.1,0),h=e(2),A=l,b=a.add(1.2),r=D(h.mul(W(A)).mul(U(b)),h.mul(W(b)),h.mul(U(A)).mul(U(b))),f=ht(u.sub(r)),v=ht(ue(ht(D(0,1,-.1)),f)),M=ht(ue(f,v)),y=ht(f.mul(1.5).add(v.mul(w.x)).add(M.mul(w.y))),R=e(.13),q=e(.3),F=e(.04),Q=m(e(.018),e(.012),c),z=r.toVar(),H=y.toVar();z.addAssign(H.mul(ut(y.add(o)).mul(.01)));const X=m(e(.3),e(1),c),E=m(e(.005),e(.02),c);m(e(.1),e(.5),c);const O=D(0,0,0).toVar(),N=e(0).toVar(),V=e(0).toVar(),Z=D(1,.55,.12),et=D(1,.3,.03),P=D(.45,.1,.01),k=D(.25,.15,.05),B=o.mul(E).mul(30);De(200,()=>{const L=ht(z),j=it(z),J=Q.mul(q).div(I(j.mul(j),e(.001))),ot=L.mul(J),st=ht(H.sub(ot)),nt=H.mul(Q);z.addAssign(nt);const mt=it(z);_(mt.lessThan(R),()=>{V.assign(1),he()});const ft=it(kt(z.x,z.z)),vt=z.y.div(F),Tt=I(e(0),e(1).sub(vt.mul(vt))),St=tt(e(1.3),e(.16),ft),bt=Tt.mul(St),At=ft.mul(4.27).sub(B),Rt=W(At),_t=U(At),dt=D(z.x.mul(Rt).sub(z.z.mul(_t)),z.y.mul(8),z.x.mul(_t).add(z.z.mul(Rt))).mul(14),Ht=se(dt).mul(.5).add(.5),Wt=se(dt.mul(2.03)).mul(.5).add(.5),Nt=se(dt.mul(4.01)).mul(.5).add(.5),Vt=Ht.mul(.25).add(Wt.mul(.12)).add(Nt.mul(.06)).add(.55),Dt=fe(z.x.negate(),z.z.negate()),Ut=e(1).add(W(Dt.add(B)).mul(.7)),qt=$(ft.add(Vt.sub(.5).mul(.4)),e(0),e(1)),Pt=m(m(Z,et,tt(e(.05),e(.425),qt)),P,tt(e(.425),e(1),qt)),Ct=I(Vt,e(.3)),Bt=Pt.mul(Ct).mul(Ut).mul(3).add(k.mul(bt).mul(2)),wt=bt.mul($(Ct.mul(2),e(0),e(1))),Jt=e(1).sub(N).mul(wt);O.assign(m(O,Bt,Jt)),N.assign($(m(N,e(1),wt),e(0),e(1))),z.addAssign(nt),H.assign(st),_(Kt(z,z).greaterThan(16).and(Kt(H,z).greaterThan(0)),()=>{he()})}),O.mulAssign(X);const C=e(1).sub(tt(e(.3),e(1),S));O.mulAssign(C);const T=I(N.mul(C),V);return It(O,T)}),p=new oe;p.transparent=!0,p.depthWrite=!1,p.side=Pe,p.fragmentNode=d(),this.mesh=new le(new Ce(1,1),p),this.mesh.scale.set(t,t,1),this.mesh.renderOrder=1}update(t,s,i,o,a,l){this.uTime.value=t,this.uTiltX.value=s,this.uRotY.value=i,this.mesh.quaternion.copy(o.quaternion);const c=o.position.length(),p=(o.fov??60)*Math.PI/180,w=a.y*l,S=this.quadSize/c*(w/(2*Math.tan(p/2)));this.uLOD.value=Math.min(Math.max((S-6)/220,0),1)}getLOD(){return this.uLOD.value}dispose(){this.mesh.material.dispose(),this.mesh.geometry.dispose(),this.depthMesh.geometry.dispose(),this.depthMesh.material.dispose()}}const Ps=`// ─── Hash functions ────────────────────────────────────────────────────────
